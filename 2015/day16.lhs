...
> parseLine str = (read . last . words $ n, map f . splitOn ',' . tail $ ps)
>   where
>     (n, ps) = span (':' /=) str
>     f x = let [a, b] = words x in (init a, read b :: Int)
>
...
>           ]
>
> isMatch = flip elem target
>
> findAunt f = fst . head . filter (and . map f . snd)
>
> day16 = solve "input-day16.txt" (findAunt isMatch . parseInput)
...
> isMatch2 ("cats",        x) = x > 7
> isMatch2 ("trees",       x) = x > 3
> isMatch2 ("pomeranians", x) = x < 3
> isMatch2 ("goldfish",    x) = x < 5
> isMatch2 x                  = isMatch x
>
> day16p2 = solve "input-day16.txt" (findAunt isMatch2 . parseInput)
