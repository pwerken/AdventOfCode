...
> makeGroups z i (x:xs)
>   | i == x        = [[x]]
>   | i < x         = makeGroups z i xs
>   | otherwise     = map (x:) (makeGroups (z - 1) (i - x) xs)
>                     ++ makeGroups z i xs
>
> oneThird    xs  = sum xs `div` 3
> isOneThird  xs  = (oneThird xs ==) . sum
> onlyThirds xs = filter (isOneThird xs) . makeGroups 6 (oneThird xs) $ xs
>
> onlyShortest xs = let s = minimum (map length xs)
>                    in filter ((s ==) . length) xs
>
> smallestQE = minimum . map product . onlyShortest
>
> parseInput = reverse . sort . map (read :: String -> Int) . lines
>
> day24 = solve "input-day24.txt" (smallestQE. onlyThirds . parseInput)
...
> oneQuart    xs  = sum xs `div` 4
> isOneQuart  xs  = (oneQuart xs ==) . sum
> onlyQuarts xs = filter (isOneQuart xs) . makeGroups 6 (oneQuart xs) $ xs
>
> day24p2 = solve "input-day24.txt" (smallestQE. onlyQuarts . parseInput)
