...
>   where
>     ls = lines i
>     ns = transpose . map (map read . words) . init $ ls
>     ops = filter (\x -> x == '+' || x == '*') . last $ ls
>
> transpose :: [[a]] -> [[a]]
> transpose ([]:_) = []
> transpose x = (map head x) : transpose (map tail x)
>
> columnOp :: Char -> [Int] -> Int
> columnOp '+' = sum
> columnOp '*' = product
>
> columnOps :: Problem -> [Int]
> columnOps = uncurry (zipWith (flip columnOp))
>
> day06p1 = solve "06" (sum . columnOps . parse)
>
> e = "123 328  51 64 \n 45 64  387 23 \n  6 98  215 314\n*   +   *   +  \n"
...
>   where
>     ls = lines i
>     ns = map (map read) . splitOn "" . map (filter (/= ' ')) . reverse . transpose . init $ ls
>     ops = reverse . filter (\x -> x == '+' || x == '*') . last $ ls
>
> day06p2 = solve "06" (sum . columnOps . parse')
