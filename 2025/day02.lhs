...
> expandRange = uncurry enumFromTo
>
> isDoublePattern :: Int -> Bool
> isDoublePattern = isDoublePattern' . show
>   where
>     isDoublePattern' :: String -> Bool
>     isDoublePattern' x
>       | odd (length x) = False
>       | otherwise      = uncurry (==) . splitAt (length x `div` 2) $ x
>
> day02p1 = solve "02" (sum . map (sum . filter isDoublePattern . expandRange) . parse)
...
>
> factors :: Int -> [Int]
> factors x = filter (\y -> x `mod` y == 0) $ enumFromTo 1 x
>
> isPattern :: Int -> Bool
> isPattern = isPattern' . show
>   where
>     isPattern' :: String -> Bool
>     isPattern' p = or . map (isPattern'' p) . init . factors . length $ p
>     isPattern'' :: String -> Int -> Bool
>     isPattern'' p i = and . mapAdjacent (==) . chuncksOf i $ p
>
> day02p2 = solve "02" (sum . map (sum . filter isPattern . expandRange) . parse)
>
> t = "11-22,95-115,998-1012,1188511880-1188511890,222220-222224,1698522-1698528,446443-446449,38593856-38593862,565653-565659,824824821-824824827,2121212118-2121212124"
