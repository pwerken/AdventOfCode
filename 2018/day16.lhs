...
>
> type Registers = [Int]
> data Opcode = ADDr Int Int Int  -- rA + rB => rC
>             | ADDi Int Int Int  -- rA + vB => rC
>             | MULr Int Int Int  -- rA * rB => rC
>             | MULi Int Int Int  -- rA * vB => rC
>             | BANr Int Int Int  -- rA & rB => rC
>             | BANi Int Int Int  -- rA & vB => rC
>             | BORr Int Int Int  -- rA | rB => rC
>             | BORi Int Int Int  -- rA | vB => rC
>             | SETr Int Int Int  -- rA      => rC
>             | SETi Int Int Int  -- vA      => rC
>             | GTir Int Int Int  -- vA > rB => rC  (1/0)
>             | GTri Int Int Int  -- rA > vB => rC  (1/0)
>             | GTrr Int Int Int  -- rA > rB => rC  (1/0)
>             | EQir Int Int Int  -- vA = rB => rC  (1/0)
>             | EQri Int Int Int  -- rA = vB => rC  (1/0)
>             | EQrr Int Int Int  -- rA = rB => rC  (1/0)
>
> getR :: Registers -> Int -> Int
> getR rs i = head $ drop (i - 1) rs
>
> setI :: Registers -> Int -> Int -> Registers
> setI (_:rs) 0 x = x : rs
> setI (r:rs) i x = r : setI rs (i - 1) x
>
> setB :: Registers -> Int -> Bool -> Registers
> setB rs i b = setI rs i $ if b then 1 else 0
>
> appOp :: Registers -> Opcode -> Registers
> appOp rs (ADDr a b c) = setI rs c $ getR rs a  +  getR rs b
> appOp rs (ADDi a b c) = setI rs c $ getR rs a  +          b
> appOp rs (MULr a b c) = setI rs c $ getR rs a  *  getR rs b
> appOp rs (MULi a b c) = setI rs c $ getR rs a  *          b
> appOp rs (BANr a b c) = setI rs c $ getR rs a .&. getR rs b
> appOp rs (BANi a b c) = setI rs c $ getR rs a .&.         b
> appOp rs (BORr a b c) = setI rs c $ getR rs a .|. getR rs b
> appOp rs (BORi a b c) = setI rs c $ getR rs a .|.         b
> appOp rs (SETr a _ c) = setI rs c $ getR rs a
> appOp rs (SETi a _ c) = setI rs c $         a
> appOp rs (GTir a b c) = setB rs c $         a  >  getR rs b
> appOp rs (GTri a b c) = setB rs c $ getR rs a  >          b
> appOp rs (GTrr a b c) = setB rs c $ getR rs a  >  getR rs b
> appOp rs (EQir a b c) = setB rs c $         a ==  getR rs b
> appOp rs (EQri a b c) = setB rs c $ getR rs a ==          b
> appOp rs (EQrr a b c) = setB rs c $ getR rs a ==  getR rs b
>
> parseP1 = map parseP1' . take 3128 . lines
