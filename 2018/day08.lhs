...
>
> data NodeTree = NT [NodeTree] [Int]
>   deriving Show
>
> parse :: String -> NodeTree
> parse = fst . listToNode . map read . words
>
> listToNode :: [Int] -> (NodeTree, [Int])
> listToNode (x:y:is) = let (xs, js) = listToNodes x is
>                           (ms, zs) = splitAt y js
>                        in (NT xs ms, zs)
>
> listToNodes :: Int -> [Int] -> ([NodeTree], [Int])
> listToNodes 0 is = ([], is)
> listToNodes i is = let (n, js) = listToNode is
>                     in mapFst (n:) $ listToNodes (i - 1) js
>
> sumAllMeta :: NodeTree -> Int
> sumAllMeta (NT ns ms) = (sum ms) + (sum . map sumAllMeta $ ns)
>
> day08 = solve "08" (sumAllMeta . parse)
...
For example, again using the above nodes:
  ...
  - Node C has one metadata entry, 2. Because node C has only one child
    ...
    of node C is 0.
  - Node A has three metadata entries: 1, 1, and 2. The 1 references node A
    's first child node, B, and the 2 references node A's second child node,
...
>
> calcCode :: NodeTree -> Int
> calcCode (NT [] ms) = sum ms
> calcCode (NT ns ms) = let js = zipWith (,) [1..] $ map calcCode $ ns
>                        in sum . map (fromMaybe 0 . flip lookup js) $ ms
>
> day08p2 = solve "08" (calcCode . parse)
