...
> react (x:xs) (y:ys)
>   | canReact x y  = react xs ys
>   | otherwise     = react (y:x:xs) ys
>
> canReact :: Char -> Char -> Bool
> canReact x y
>   | x == y    = False
>   | otherwise = (toLower x) == (toLower y)
>
> day05 = solve "05" (length . react [] . filter isLetter)
...
>
> reactWithout :: String -> [Int]
> reactWithout = zipWith (\x -> length . react "" . removeUnits x) allUnits . repeat
>   where
>     allUnits :: [String]
>     allUnits = map (\u -> [u, toUpper u]) ['a'..'z']
>
>     removeUnits :: String -> String -> String
>     removeUnits us = filter (not . flip elem us)
>
> day05p2 = solve "05" (minimum . reactWithout . filter isLetter)
