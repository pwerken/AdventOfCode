...
>
> type Prereqs = [(Char, String)]
>
> parse :: String -> Prereqs
> parse = let parse' = mapSnd (head . drop 30) . mapFst head . splitAt 1
>          in buildReqList . map (parse' . drop 5) . lines
>
> updateReqList :: Char -> String -> Prereqs -> Prereqs
> updateReqList a b []            = [(a, b)]
> updateReqList a b ((x, ys):zs)
>   | a == x    = (x, b ++ ys):zs
>   | otherwise = (x, ys):updateReqList a b zs
>
> buildReqList :: [(Char, Char)] -> Prereqs
> buildReqList rs = reqList'' missing . flip reqList' [] $ rs
>   where
>     missing = (map fst rs) \\ (map snd rs)
>
>     reqList' :: [(Char, Char)] -> Prereqs -> Prereqs
>     reqList'  []           rl   = rl
>     reqList'  ((b, a):zs)  rl   = reqList' zs (updateReqList a [b] rl)
>
>     reqList'' :: String -> Prereqs -> Prereqs
>     reqList'' []     rl = rl
>     reqList'' (x:xs) rl = reqList'' xs (updateReqList x [] rl)
>
> removeTask :: Char -> Prereqs -> Prereqs
> removeTask s = filter ((/=) s . fst)
>
> removePrereq :: Char -> Prereqs -> Prereqs
> removePrereq s = map (mapSnd (filter ((/=) s))) . removeTask s
>
> validSteps :: String -> Prereqs -> String
> validSteps d = map fst . filter (prereqCheck d . snd)
>   where
>     prereqCheck :: String -> String -> Bool
>     prereqCheck x y = y == (y `intersect` x)
>
> resolveReqs :: String -> Prereqs -> String
> resolveReqs d [] = reverse d
> resolveReqs d rl = let s = head . sort $ validSteps d rl
>                     in resolveReqs (s:d) (removePrereq s rl)
>
> day07 = solve "07" (resolveReqs "" . parse)>
...
>
> type Work = (Int, String, Prereqs, [(Int, Char)])
>
> taskTime :: Char -> Int
> taskTime = flip (-) 4 . ord
>
...
> doWork ps = i
>   where
>     (i, _, _, _) = resolveTasks (0, "", ps, [])
>     resolveTasks :: Work -> Work
>     resolveTasks (i, d, [], []) = (i, d, [], [])
>     resolveTasks w = resolveTasks . completeTasks . assignTasks $ w
>
> assignTasks :: Work -> Work
> assignTasks (i, d, ps, ws)  = let vs = take (5 - length ws) $ validSteps d ps
>                                   ts = map (\c -> (i + taskTime c, c)) $ vs
>                                   rs = foldl (flip removeTask) ps vs
>                                in (i, d, rs, sortWith fst (ts ++ ws))
>
> completeTasks :: Work -> Work
> completeTasks (_, d, ps, ws)  = let i  = fst . head $ ws
>                                     (xs, ys) = partition ((==) i . fst) ws
>                                     zs = map snd xs
>                                  in (i, sort (d ++ zs), ps, ys)
>
> day07p2 = solve "07" (doWork . parse)
