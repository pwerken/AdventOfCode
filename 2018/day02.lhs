...
> parse = lines
>
> countSame :: Ord a => [a] -> [Int]
> countSame = map length . group . sort
>
> only23 :: String -> [Int]
> only23 = nub . filter (`elem` [2,3]) . countSame
>
> day02 = solve "02" (product . countSame . concatMap only23 . lines)
...
>
> findAllSame :: Ord a => [[a]] -> [[a]]
> findAllSame []     = []
> findAllSame (x:xs) = (map (same x) xs) ++ (findAllSame xs)
>
> day02p2 = solve "02" (head . filter ((== 25) . length). findAllSame . lines)
