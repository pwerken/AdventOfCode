...
>
> type Square = (Int, Int, Int) -- x,y,size
> type Pos    = (Int, Int)      -- x,y
>
> charge :: Int -> Int -> Int -> Int
> charge s x y = (((10 + x) * y + s) * (10 + x) `div` 100) `mod` 10 - 5
>
> squares :: Int -> [Square]
> squares s = topLefts (1, 1, s)
>
> topLefts :: Square -> [Square]
> topLefts (x, y, s)
>   | (y + s - 1 > 300) = []
>   | (x + s - 1 > 300) = topLefts (1, y + 1, s)
>   | otherwise         = (x, y, s) : topLefts (x + 1, y, s)
>
> sqToPos :: Square -> [Pos]
> sqToPos (x, y, s) = let xs = concat . replicate s . take s $ [0..]
>                         ys = concat . take s . map (replicate s) $ [0..]
>                      in zipWith (\a b -> (x + a, y + b)) xs ys
>
> squareCharge :: Square -> Int
> squareCharge = sum . map (uncurry (charge 4151)) . sqToPos
>
> maxSquare :: [Square] -> Square
> maxSquare = maxWith squareCharge
>
> day11 = (\(x,y,_) -> (x,y)) . maxSquare . squares $ 3
...
>
> day11p2 = maxSquare . squares $ 14
>
>
> main = putStrLn . show . map (f . maxSquare . squares) [3..300]
>   where
>     f x = (x, squareCharge x)
