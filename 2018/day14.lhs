...
> type Elves  = (Int, Int)
> type State  = (Elves, Int, Scores)
>
> start :: State
> start = ((0, 1), 2, Seq.fromList [3, 7])
>
> scores :: [Int]
> scores = 3 : 7 : nextScore start
>   where
>     nextScore :: State -> [Int]
>     nextScore ((x, y), i, ss) =
>         let a  = ss `Seq.index` x -- (IA.!) ss x -- 
>             b  = ss `Seq.index` y -- (IA.!) ss y -- 
>             cs = getScores (splitInt (a + b))
>             ts = foldl (Seq.|>) ss cs
>             j  = i + length cs
>          in cs ++ nextScore ((moveElf j (x+1+a), moveElf j (y+1+b)), j, ts)
>
>     splitInt :: Int -> (Int, Int)
>     splitInt i = let a = i `div` 10
>                   in (a, i - 10 * a)
>
...
>
> moveElf :: Int -> Int -> Int
> moveElf m x = if x < m then x else moveElf m (x - m)
>
> after :: Int -> String
> after = concatMap show . take 10 . flip drop scores
>
> day14 :: String
> day14 = after 909441
...
>
> untilScore :: [Int] -> [Int] -> Int
> untilScore xs ys@(_:zs)
>   | or (zipWith (/=) xs ys) = (+) 1 $ untilScore xs zs
>   | otherwise               = 0
>
> day14p2 = flip untilScore scores $ [9,0,9,4,4,1]
