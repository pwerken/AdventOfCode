...
will have a plant in the next generation. For example:
  ...
  - A note like .##.# => # means that a pot has a plant in a given
    ...
    pot, the one immediately to the left, and the one two pots to the right,
...
> import Data.Maybe
>
> type Plants   = (Integer, [Char])
> type Patterns = [(String, Char)]
> type State    = (Plants, Patterns)
>
> parse :: String -> State
> parse = let f = (,) 0 . drop 15 . head
>             g = map (mapSnd (head . drop 3) . split ' ')
>          in mapFst f . mapSnd g . split "" . lines
>
> group5 (a:b:c:d:e:zs) = [a,b,c,d,e] : group5 (b:c:d:e:zs)
> group5 _              = []
>
> get :: Eq a => [(a, b)] -> a -> b
> get xs = fromJust . flip lookup xs
>
> trim :: Plants -> Plants
> trim (i, ('.':xs))  = trim ((i + 1), xs)
> trim (i,      xs)   = (i, reverse . dropWhile (== '.') . reverse $ xs)
>
> grow :: Patterns -> Plants -> Plants
> grow ps (i, as) = trim (i - 2, map (get ps) $! group5 ("...." ++ as ++ "...."))
>
> growForever :: Patterns -> Plants -> [Plants]
> growForever ps = iterate (grow ps)
>
> generation :: Int -> State -> Plants
> generation x (ys, ps) = head . drop x . growForever ps $ ys
>
> sumPots :: Plants -> Integer
> sumPots (i, as) = let bs = zipWith (,) as [i..]
>                    in sum . map snd . filter ((==) '#' . fst) $ bs
>
> day12 = solve "12" (sumPots . generation 20 . parse)
...
>
> extendCycle :: Integer -> [Plants] -> Integer
> extendCycle i xs
>   | i < 300     = sumPots . head . drop (fromInteger i) $ xs
>   | otherwise   = let (a:b:_) = drop 300 xs
>                       x = sumPots a
>                       d = sumPots b - x
>                    in x + d * (i - 300)
>
> plants :: (Plants, Patterns) -> [Plants]
> plants = uncurry (flip growForever)
>
> day12p2 = solve "12" (extendCycle 50000000000 . plants . parse)
