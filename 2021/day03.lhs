...
>
> freq1 :: [[Int]] -> [Int]
> freq1 = foldl1 (zipWith (+)) . map (1:)
>
> bin2int :: [Bool] -> Int
> bin2int = sum . zipWith (\x b -> if b then 2^x else 0) [0..] . reverse
>
> gamma :: [Int] -> Int
> gamma (l:xs) = bin2int . map ((<) l . (*) 2) $ xs
>
> epsilon :: [Int] -> Int
> epsilon (l:xs) = bin2int . map ((>) l . (*) 2) $ xs
>
> resultP1 xs = gamma xs * epsilon xs
>
> day03 = solve "03" (resultP1 . freq1 . parse)
>
>
>
> exampleP1 = unlines
...
>
> select :: (Int -> Int -> Bool) -> ([a], [a]) -> [a]
> select f (as, bs) = if length as `f` length bs then as else bs
>
> dive :: (Int -> Int -> Bool) -> [[Bool]] -> [Bool]
> dive _ [x] = x
> dive f xs  = (head $ head ys) : dive f (map tail ys)
>   where
>     ys = select f . partition (head) $ xs
>
> resultP2 :: [[Int]] -> Int
> resultP2 is = o2 * co2
>   where
...
>     co2 = bin2int . dive (<)  $ bs
>
> day03p2 = solve "03" (resultP2 . parse)
