...
virus carrier is always located on a single node in the network (the current
...
> type Pos = (Int, Int)
>
> data Cell = C | I | W | F deriving (Eq, Show)
> type Grid = [(Pos, Cell)]
>
> type Virus = (Pos, Int)
> type State = (Grid, Virus)
>
> parse :: String -> Grid
> parse = filter ((==) I . snd) . concat . zipWith parse' [-12..12] . lines
>   where
>     inf c = if c == '#' then I else C
>     parse' y = map (mapSnd inf) . zipWith (parse'' y) [-12..12]
>     parse'' y x c = ((x, y), c)
>
> start :: Grid -> State
> start g = (g, ((0,0), 0))
>
> getCell :: Pos -> Grid -> Cell
> getCell p = maybe C id . lookup p
>
...
>
> moveVirus (x, y) 0 = ((x    , y - 1), 0)
> moveVirus (x, y) 1 = ((x + 1, y    ), 1)
> moveVirus (x, y) 2 = ((x    , y + 1), 2)
> moveVirus (x, y) 3 = ((x - 1, y    ), 3)
>
> burst :: State -> State
...
>
> countInf :: State -> Int
> countInf = length . filter ((==) I) . map snd . fst
>
> day22 = solve "22" (countInf . bursts 10000 . start . parse)
...
as W and flagged as F, the middle of the infinite grid looks like this, with
...
feature of this evolved virus is speed; of the first dayday bursts,
