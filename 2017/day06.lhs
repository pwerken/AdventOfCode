...
>
> looper :: [Int] -> [[Int]]
> looper = looper' [] . iterate redist
>   where
>     looper' zs (x:xs)
>       | any (== x) zs = [x]
>       | otherwise     = x : looper' (x:zs) xs
>
> redist :: [Int] -> [Int]
> redist xs = take l . drop (l - i) . cycle . incrList x y $ rs
>   where
>     l = length xs
>     m = maximum xs
>     i = (1 +) . length . takeWhile (/= m) $ xs
>     x = m `div` l
>     y = m `mod` l
>     rs = (++ [0]) . tail . take l . dropWhile (/= m) . cycle $ xs
>
