...
> parse = concatMap (parse' . words) . lines . filter (/= ',')
>   where
>     parse' (x:_:ys) = map ((,) (read x) . read) ys
>
> buildTree :: Int -> [(Int, Int)] -> Tree Int
> buildTree l = Node l . uncurry (flip map)
>                      . mapFst (map snd)
>                      . mapSnd (flip buildTree . filter ((/=) l . snd))
>                      . partition ((==) l . fst)
>
> countPs :: Eq a => Tree a -> Int
> countPs = length . nub . flatten
>
> day12 = solve "12" (countPs . buildTree 0 . parse)
...
>
> countGrps []             = 0
> countGrps xs@((x, _):_)  = 1 + countGrps zs
>   where
>     ys = flatten $ buildTree x xs
>     zs = filter (not . flip elem ys . fst) xs
>
> day12p2 = solve "12" (countGrps . parse)
