...
> import Helpers
>
> data Tape = Tape { le :: [Int], cur :: Int, ri :: [Int] }
>
> count :: Tape -> Int
> count (Tape b c a) = sum b + c + sum a
>
> right :: Int -> Tape -> Tape
> right n (Tape l _ []) = Tape (n:l) 0        []
> right n (Tape l _ r ) = Tape (n:l) (head r) (tail r)
>
> left :: Int -> Tape -> Tape
> left n (Tape [] _ r) = Tape []       0        (n:r)
> left n (Tape l  _ r) = Tape (tail l) (head l) (n:r)
>
> pA :: Int -> Tape -> Int
> pA 0 tp               = count tp
> pA c tp@(Tape _ 0 _)  = pB (c - 1) (right 1 tp)
> pA c tp@(Tape _ 1 _)  = pE (c - 1) (left  0 tp)
>
> pB :: Int -> Tape -> Int
> pB 0 tp               = count tp
> pB c tp@(Tape _ 0 _)  = pC (c - 1) (left  1 tp)
> pB c tp@(Tape _ 1 _)  = pA (c - 1) (right 0 tp)
>
> pC :: Int -> Tape -> Int
> pC 0 tp               = count tp
> pC c tp@(Tape _ 0 _)  = pD (c - 1) (left  1 tp)
> pC c tp@(Tape _ 1 _)  = pC (c - 1) (right 0 tp)
>
> pD :: Int -> Tape -> Int
> pD 0 tp               = count tp
> pD c tp@(Tape _ 0 _)  = pE (c - 1) (left  1 tp)
> pD c tp@(Tape _ 1 _)  = pF (c - 1) (left  0 tp)
>
> pE :: Int -> Tape -> Int
> pE 0 tp               = count tp
> pE c tp@(Tape _ 0 _)  = pA (c - 1) (left  1 tp)
> pE c tp@(Tape _ 1 _)  = pC (c - 1) (left  1 tp)
>
> pF :: Int -> Tape -> Int
> pF 0 tp               = count tp
> pF c tp@(Tape _ 0 _)  = pE (c - 1) (left  1 tp)
> pF c tp@(Tape _ 1 _)  = pA (c - 1) (right 1 tp)
>
> day25 = pA 12386363 $ Tape [] 0 []
>
>
>
> eA :: Int -> Tape -> Int
> eA 0 tp               = count tp
> eA c tp@(Tape _ 0 _)  = eB (c - 1) (right 1 tp)
> eA c tp@(Tape _ 1 _)  = eB (c - 1) (left  0 tp)
>
> eB :: Int -> Tape -> Int
> eB 0 tp               = count tp
> eB c tp@(Tape _ 0 _)  = eA (c - 1) (left  1 tp)
> eB c tp@(Tape _ 1 _)  = eA (c - 1) (right 1 tp)
