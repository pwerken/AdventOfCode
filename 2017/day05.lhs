...
> import qualified Data.Vector.Unboxed as V (length, fromList)
>
> type P = (Int, Vector Int)
>
> parse :: String -> P
> parse = (,) 0 . V.fromList . map read . lines
>
> active :: P -> Bool
> active (p, is) = p >= 0 && p < V.length is
>
> run :: (Int -> Int) -> P -> [Int]
> run f (p, is) = if active (p, is) then p : run f (q, js) else []
>   where
>     i = is ! p
>     q = p + i
>     js = is // [(p, f i)]
