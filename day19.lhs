...
>             . (\[x,y] -> (x,y)) . splitOn "" . lines)
>
> replace1 _     []     = [[]]
> replace1 (x,y) (z:zs)
>     | a /= x          = map (z:) (replace1 (x,y) zs)
>     | otherwise       = cs ++ [y ++ bs]
>   where
>     l       = if length x > length (z:zs) then 0 else length x
>     (a,bs)  = splitAt l (z:zs)
>     cs      = map (a ++) $ replace1 (x, y) bs
>
> replace1Map (rs, m) = nub . concat $ map (tail . flip replace1 m) rs
>
> day19 = solve "input-day19.txt" (length . replace1Map . parseInput)
