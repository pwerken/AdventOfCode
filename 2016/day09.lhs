...
>
> decomp :: String -> Int
> decomp ('(':xs) = let (ys, zs) = split ')' xs
>                       (s,  t)  = mapFst read . mapSnd read $ split 'x' ys
>                    in (s * t) + (decomp $ drop s zs)
> decomp (x:xs)   = 1 + decomp xs
> decomp []       = 0
>
> day09 = solve "input-day09.txt" (show . decomp . init)
...
> decomp2 :: String -> Int
> decomp2 ('(':xs)  = let (ys, zs) = split ')' xs
>                         (s,  t)  = mapFst read . mapSnd read $ split 'x' ys
>                      in ((decomp2 $ take s zs) * t) + (decomp2 $ drop s zs)
> decomp2 (x:xs)    = 1 + decomp2 xs
> decomp2 []        = 0
>
> day09p2 = solve "input-day09.txt" (show . decomp2 . init)
