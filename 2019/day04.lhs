...
> hasDouble = or . map ((2 <=) . length) . groupBy (==)
>
> countMatches f (lo, hi) = length . filter f . filter increases . allPossible $ lo
>   where
>     increases :: String -> Bool
>     increases xs = and $ zipWith (<=) xs (tail xs)
>
>     allPossible :: String -> [String]
>     allPossible = takeWhile (/= hi) . map reverse . iterate next . reverse
>
>     next :: String -> String
>     next ('9':xs) = '0' : next xs
>     next (x:xs) = (chr . (+) 1 . ord $ x) : xs
>
> day04 = solve "04" (countMatches hasDouble . parse)
...
> hasStrictDouble = or . map ((2 ==) . length) . groupBy (==)
>
> day04p2 = solve "04" (countMatches hasStrictDouble . parse)
