...
>
> type Intcode = (Int, Map.Map Int Int)
>
> parse :: String -> Intcode
> parse = (,) 0 . Map.fromList . zipWith (,) [0..] . map read . splitOn ','
>
> modState :: (Int, Int) -> Intcode -> Intcode
> modState (x, y) = mapSnd (Map.insert 1 x . Map.insert 2 y)
>
> runProgram :: Intcode -> Int
> runProgram (i, xs) =
>   let o = xs Map.! i
>       a = xs Map.! (xs Map.! (i + 1))
>       b = xs Map.! (xs Map.! (i + 2))
>       y = xs Map.! (i + 3)
>    in case o of
>       1   -> runProgram (i + 4, Map.insert y (a+b) xs)
>       2   -> runProgram (i + 4, Map.insert y (a*b) xs)
>       99  -> xs Map.! 0
>
> runMod :: (Int, Int) -> Intcode -> Int
> runMod m = runProgram . modState m
>
> day02 = solve "02" (runMod (12, 2) . parse)
...
>
> nounsVerbs :: [(Int, Int)]
> nounsVerbs = [(x, y) | x <- [0..99], y <- [0..99]]
>
> findOutput :: Int -> Intcode -> (Int, Int)
> findOutput t p = fromJust . find ((==) t. flip runMod p) $ nounsVerbs
>
> toAnswer :: (Int, Int) -> Int
> toAnswer = uncurry (+) . mapFst ((*) 100)
>
> day02p2 = solve "02" (toAnswer . findOutput 19690720 . parse)
