...
> type Panel = Map.Map Pos [(Int, Int)]
>
> addPos :: Pos -> Pos -> Pos
> addPos (x1, y1) (x2, y2) = (x1 + x2, y1 + y2)
>
> parse :: String -> [Wire]
> parse = map (map (mapSnd read . fromJust . uncons) . splitOn ',') . lines
>
> wire2grid :: Wire -> Grid
> wire2grid = let dir2pos :: Char -> Pos
>                 dir2pos 'U' = ( 0, 1)
>                 dir2pos 'D' = ( 0,-1)
>                 dir2pos 'L' = (-1, 0)
>                 dir2pos 'R' = ( 1, 0)
>                 edge2list = concatMap (uncurry (flip replicate))
>                 wire2pos :: Wire -> [Pos]
>                 wire2pos = tail . scanl addPos (0, 0) . map dir2pos . edge2list
>                 addSteps = zipWith (flip (,)) [1..]
>              in Map.fromListWith min . addSteps . wire2pos
>
> grids2panel :: [Grid] -> Panel
> grids2panel = let toPanel :: Int -> Grid -> Panel
>                   toPanel i = Map.map ((:[]) . (,) i)
>                   f g (x:y:_) = g x y
>                in f (Map.unionWith (++)) . zipWith toPanel [1..]
>
> crosses :: [Wire] -> Panel
> crosses = Map.filter ((== 2) . length) . grids2panel . map wire2grid
>
> dist :: Pos -> Int
> dist (x, y) = abs(x) + abs(y)
>
> nearest :: Panel -> Int
> nearest = dist . head . sortWith dist . Map.keys
>
> day03 = solve "03" (nearest . crosses . parse)
...
>
> fastest :: Panel -> Int
> fastest = timing . snd . head . sortOn (timing . snd) . Map.toList
>
> day03p2 = solve "03" (fastest . crosses . parse)
>
